...
- NEW: Respects max slots per day preference from Nombre-Max column
...
- SOFT (Priority 4): Respect max slots per day preference (weight 120)
...
class DataImporter:
    ...
    def import_unavailability(filepath: str, teachers: List[Teacher],
        ...
        - Column 'Nombre-Max': Maximum slots per day (1-4)
        """
        df = pd.read_excel(filepath)
        ...
        # First pass: collect max_slots_per_day for each teacher
        teacher_max_slots = {}  # teacher_id -> max_slots
        for idx, row in df.iterrows():
            try:
                teacher_name = str(row['Enseignant']).strip().lower()
                teacher_id = name_to_id.get(teacher_name)

                if teacher_id and teacher_id in teacher_dict:
                    # Get max slots per day (default to 4 if not specified or invalid)
                    if 'Nombre-Max' in row and pd.notna(row['Nombre-Max']):
                        max_slots = int(row['Nombre-Max'])
                        max_slots = max(1, min(4, max_slots))  # Ensure between 1-4
                        teacher_max_slots[teacher_id] = max_slots
            except (ValueError, TypeError, KeyError):
                continue

        # Second pass: process unavailability constraints
        for idx, row in df.iterrows():
            try:
        ...
        # Apply max_slots_per_day to teachers
        for teacher_id, max_slots in teacher_max_slots.items():
            if teacher_id in teacher_dict:
                teacher_dict[teacher_id].max_slots_per_day = max_slots
                max_slots_set += 1
        ...
        print(f"  - Teachers with max slots/day limit: {max_slots_set}/{len(teachers)}")
...
class SlotBasedScheduler:
    ...
    def _add_soft_constraints(self):
        ...
        # 4. PRIORITY 4: Respect max slots per day preference (weight 120)
        # This is a SOFT preference - violations are allowed but penalized
        max_slots_penalty = 0
        for teacher_id, teacher in self.teachers.items():
            if teacher.max_slots_per_day < 4:  # Only if there's a restriction
                for day in all_days:
                    # Get slots that exist for this day
                    day_slots = [(day, ts.slot) for ts in self.time_slots if ts.day == day]

                    if day_slots:
                        # Count how many slots teacher works on this day
                        slots_worked = self.model.NewIntVar(0, 4, f'slots_worked_{teacher_id}_d{day}')
                        self.model.Add(slots_worked == sum(
                            self.assignments[(teacher_id, key)] for key in day_slots
                        ))

                        # Penalty for exceeding max_slots_per_day (progressive penalty)
                        # Each slot over the limit adds to the penalty
                        excess = self.model.NewIntVar(0, 4, f'excess_{teacher_id}_d{day}')
                        self.model.Add(excess >= slots_worked - teacher.max_slots_per_day)
                        self.model.Add(excess >= 0)

                        # Progressive penalty: 80 for first violation, +40 for each additional
                        penalties.append(excess * 80)
                        max_slots_penalty += 1

        if max_slots_penalty > 0:
            print(f"  ✓ Priority 4: Respect max slots/day preference (weight 120) - {max_slots_penalty} teacher-day combinations")
        ...
        if any(t.max_slots_per_day < 4 for t in self.teachers.values()):
            print("    4. Respect max slots per day preference (weight 120)")
    ...
    def print_solution(self):
        ...
        for grade in sorted(set(t.grade for t in self.teachers.values())):
            ...
            for teacher in sorted(teachers_in_grade, key=lambda t: t.id):
                ...
                violation_marker = ""
                if teacher.max_slots_per_day < 4:
                    # Count slots per day
                    day_slot_counts = defaultdict(int)
                    for (day, slot) in self.solution['teacher_slots'][teacher.id]:
                        day_slot_counts[day] += 1

                    violations = [day for day, count in day_slot_counts.items() if count > teacher.max_slots_per_day]
                    if violations:
        ...
        # Check max slots per day summary
        teachers_with_limit = [t for t in self.teachers.values() if t.max_slots_per_day < 4]
        if teachers_with_limit:
            violations_count = 0
            for teacher in teachers_with_limit:
                day_slot_counts = defaultdict(int)
                for (day, slot) in self.solution['teacher_slots'][teacher.id]:
                    day_slot_counts[day] += 1
            ...
            print(f"Max slots/day preference: {len(teachers_with_limit) - violations_count}/{len(teachers_with_limit)} teachers within limit")
...
if __name__ == "__main__":
    ...
    if scheduler.solve(time_limit=30):
        ...
        print("  - Max slots per day preference for each teacher")
        ...
        print("  4. Max slots per day constraints too restrictive")
        print("\nSuggestions:")
